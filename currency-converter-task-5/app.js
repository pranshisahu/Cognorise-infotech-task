...
let getBtn = documnet.querySelector("button");
...
for(let i=0; i < dropList.length; i++){
    ...
    function loadFlag(element){
        for(let code  in  country_list){
            if(code == element.value){
                ...
                imgTag.src = `https://flagcdn.com/48*36/${country_list[code].toLowerCase()}.png`
            }
        }
    }
    ...
    function getExchangeValue() {
        ...
        let url = `https://v6.exchangerate-api.com/v6/132628fdf509/latest/${fromCurrency.value}`;
